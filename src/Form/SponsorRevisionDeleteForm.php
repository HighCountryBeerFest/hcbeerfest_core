...
class SponsorRevisionDeleteForm extends ConfirmFormBase {
   ...
   *
   * @var \Drupal\hcbeerfest_core\Entity\SponsorInterface
   */
  protected $revision;
  ...
  public function getCancelUrl() {
    return new Url('entity.sponsor.version_history', array('sponsor' => $this->revision->id()));
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->logger('content')->notice('Sponsor: deleted %title revision %revision.', array('%title' => $this->revision->label(), '%revision' => $this->revision->getRevisionId()));
    drupal_set_message(t('Revision from %revision-date of Sponsor %title has been deleted.', array('%revision-date' => format_date($this->revision->getRevisionCreationTime()), '%title' => $this->revision->label())));
    $form_state->setRedirect(
      'entity.sponsor.canonical',
       array('sponsor' => $this->revision->id())
    );
    if ($this->connection->query('SELECT COUNT(DISTINCT vid) FROM {sponsor_field_revision} WHERE id = :id', array(':id' => $this->revision->id()))->fetchField() > 1) {
      $form_state->setRedirect(
