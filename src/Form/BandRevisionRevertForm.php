...
/**
 * Provides a form for reverting a Band revision.
 *
 * @ingroup hcbeerfest_core
 */
class BandRevisionRevertForm extends ConfirmFormBase {
  ...
  /**
   * Constructs a new BandRevisionRevertForm.
   *
  ...
  public function getFormId() {
    return 'band_revision_revert_confirm';
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->logger('content')->notice('Band: reverted %title revision %revision.', ['%title' => $this->revision->label(), '%revision' => $this->revision->getRevisionId()]);
    drupal_set_message(t('Band %title has been reverted to the revision from %revision-date.', ['%title' => $this->revision->label(), '%revision-date' => $this->dateFormatter->format($original_revision_timestamp)]));
   ...
   *
   * @param \Drupal\hcbeerfest_core\Entity\BandInterface $revision
